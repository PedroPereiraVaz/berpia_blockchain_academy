...
class SlideChannel(models.Model):
    ...
    @api.onchange(
        "blockchain_certification_enabled",
        "enroll",
        "product_id",
        "blockchain_certification_price",
    )
    def _onchange_blockchain_config_validation(self):
        ...
        for record in self:
            ...
            if record.blockchain_certification_enabled and not record.product_id:
                _logger.info(
                    "Se creará un producto automáticamente para la certificación blockchain de '%s'",
                    record.name,
                )
    ...
    def _get_blockchain_attribute(self):
        ...
        # Valor estandar
        val_std = AttributeValue.search(
            [("attribute_id", "=", attr.id), ("name", "=", "Estándar")], limit=1
        )
        if not val_std:
            val_std = AttributeValue.create(
                {"attribute_id": attr.id, "name": "Estándar", "sequence": 1}
            )

        # Valor certificado
        val_cert = AttributeValue.search(
            [("attribute_id", "=", attr.id), ("name", "=", "Certificado Blockchain")],
            limit=1,
        )
        if not val_cert:
            val_cert = AttributeValue.create(
                {
    ...
    def _sync_course_product(self):
        ...
        for channel in self:
            ...
            # berpia_academy solo crea producto si enroll == 'payment'.
            # Aquí forzamos la creación si hay precio de certificación.
            if (
                not channel.product_id
                and channel.blockchain_certification_enabled
                and channel.blockchain_certification_price > 0
            ):
                product_vals = {
                    "name": channel.name,
    ...
    def _action_add_members(self, target_partners):
        ...
        for channel in self:
            if (
                channel.blockchain_certification_enabled
                and channel.blockchain_certification_price == 0
            ):
